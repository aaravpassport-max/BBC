...
const wideSpread = api.simulateOrderRejection({ askQty: 5000, bidQty: 5000, bidprice: 100, askPrice: 113 }, 50, 'buy', { scalpingProfitProfile: true, spreadHardBlockPct: 12 });
assert.strictEqual(wideSpread.rejected, true, 'spread above 12% must reject under scalping profile');

const relaxedSpread = api.simulateOrderRejection({ askQty: 5000, bidQty: 5000, bidprice: 100, askPrice: 112.5 }, 50, 'buy', { scalpingProfitProfile: true, spreadHardBlockPct: 13 });
assert.strictEqual(relaxedSpread.rejected, false, '12.5% spread should pass relaxed 13% block');
