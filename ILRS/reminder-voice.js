/**
 * Natural Indian English reminder announcements — clear, familiar tone.
 */
...
function buildIndianEnglishAnnouncement(item, type = 'reminder') {
  if (!item) return '';
...
function getAnnouncementDisplayText(item, type = 'reminder') {
  const spoken = buildIndianEnglishAnnouncement(item, type);
  return spoken || clean(item.why_it_matters) || clean(item.title) || 'Time for your reminder.';
}

module.exports = {
  buildIndianEnglishAnnouncement,
  getAnnouncementDisplayText,
};
